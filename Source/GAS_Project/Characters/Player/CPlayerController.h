...
protected:
	//  “처형 가능” 프롬프트 위젯 클래스(BP에서 지정)
	UPROPERTY(EditDefaultsOnly, Category="UI|Execution")
	TSubclassOf<UUserWidget> ExecutionWidgetClass;

	//  로컬에서 생성되는 실제 위젯 인스턴스
	UPROPERTY(Transient)
	TObjectPtr<UUserWidget> ExecutionWidget;

	//  서버가 “이 Enemy는 처형 UI 켜도 됨”이라고 통지한 후보들
	UPROPERTY(Transient)
	TSet<TObjectPtr<class ACEnemyBase>> ExecutionCandidates;
