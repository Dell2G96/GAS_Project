...
UAsyncAction_ObserveTeam* UAsyncAction_ObserveTeam::InternalObserveTeamChanges(
	TScriptInterface<ILeeTeamAgentInterface> TeamActor)
{
...
void UAsyncAction_ObserveTeam::SetReadyToDestroy()
{
	...
	if (ILeeTeamAgentInterface* TeamInterface = TeamInterfacePtr.Get())
	{
...
void UAsyncAction_ObserveTeam::Activate()
{
	...
	int32 CurrentTeamIntex = INDEX_NONE;

	if (ILeeTeamAgentInterface* TeamInterfaece = TeamInterfacePtr.Get())
	{
		CurrentTeamIntex = GenericTeamIdToInteger(TeamInterfaece->GetGenericTeamId());

		TeamInterfaece->GetTeamChangedDelegateChecked().AddDynamic(this, &ThisClass::OnWatchedAgentChangedTeam);

		bCouldSucceed = true;
	}

	OnTeamChanged.Broadcast(CurrentTeamIntex != INDEX_NONE, CurrentTeamIntex);
